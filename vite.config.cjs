...
exports.default = (0, vite_1.defineConfig)(async () => {
    ...
    if (process.env.NODE_ENV !== "production" && process.env.REPL_ID !== undefined) {
        try {
            const { default: runtimeErrorOverlay } = await Promise.resolve().then(() => __importStar(require("@replit/vite-plugin-runtime-error-modal")));
            const { cartographer } = await Promise.resolve().then(() => __importStar(require("@replit/vite-plugin-cartographer")));
            const { devBanner } = await Promise.resolve().then(() => __importStar(require("@replit/vite-plugin-dev-banner")));
