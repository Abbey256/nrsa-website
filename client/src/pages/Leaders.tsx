...
export default function Leaders() {
  ...
  const handleReadBio = (leaderId: number) => navigate(`/leaders/${leaderId}`);
  ...
  const renderContent = () => {
    ...
    if (leaders.length === 0) return (
      <div className="text-center p-12 bg-yellow-50 border border-yellow-300 rounded-lg shadow-md">
        ...
        <p className="mt-2 text-md text-yellow-600">
          Leader profiles will appear here once added in the admin dashboard.
        </p>
      </div>
    );
        ...
        {leaders.map((leader) => (
          ...
          >
            ...
            <CardContent className="pt-6 text-center">
              <h3 className="text-xl font-extrabold text-gray-900">{leader.name}</h3>
